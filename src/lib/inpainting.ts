...
export function buildStepBodyPrompt(
  ...
  const system = [
    ...
    TONE_RULES,
    '',
    'Each step body follows this exact internal structure:',
    '',
    "- Start with a Basket: 2-3 sentences bridging from the previous step. What did the learner just do? Why does this next step follow?",
    '- Then an Objective: one clear sentence stating what the learner will accomplish in this step.',
    '- Then Key Actions: a numbered list of concrete substeps. Each substep should include what to do, any specific commands or settings, and the expected observable result. Use fenced code blocks with language tags for commands, code, and configuration files.',
    "- End with a Launchpad: 1-2 sentences bridging to the next step. What's coming next and why?",
    '',
    'Format rules:',
    '- Use markdown.',
    '- Label each section inline with bold: **Basket**:, **Objective**:, **Key Actions**:, **Launchpad**:. Do not use ### headings for these section labels — they conflict with the step title above the body.',
    '- Use ordered lists (1., 2., 3.) for Key Actions. Use unordered lists only when substeps are parallel, not sequential.',
    '- Use fenced code blocks with language tags (```bash, ```python, ```json, etc.) for commands, code, and configuration.',
    '- Use inline backticks for file paths, environment variable names, CLI flags, and short identifiers.',
    '- Use blockquotes (> **Tip:** ...) for tips and notes. Use (> **Warning:** ...) for cost, security, or destructive-action warnings.',
    '- Never use markdown tables — the UI does not render them.',
    '- Every step must be action-focused: the learner is building, running, configuring, or creating something tangible. Avoid "understand" or "think about" as primary verbs.',
...
export function buildInpaintingPrompt(
  ...
  const system = [
    ...
    TONE_RULES,
    '',
    'Each step body follows this exact internal structure — preserve it under any transform:',
    '',
    "- Start with a Basket: 2-3 sentences bridging from the previous step. What did the learner just do? Why does this next step follow?",
    '- Then an Objective: one clear sentence stating what the learner will accomplish in this step.',
    '- Then Key Actions: a numbered list of concrete substeps. Each substep should include what to do, any specific commands or settings, and the expected observable result. Use fenced code blocks with language tags for commands, code, and configuration files.',
    ...
    '- Every step must be action-focused: the learner is building, running, configuring, or creating something tangible. Avoid "understand" or "think about" as primary verbs.',
    '- Do not repeat the step heading in the body.',
    '- No meta commentary like "here is the rewritten step".',
  ].join('\n')
